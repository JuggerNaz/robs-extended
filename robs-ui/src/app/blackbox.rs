...
//! threads and is intentionally decoupled from the egui loop. This module is
//! the thin adapter on [`RobsApp`] that:
//!
...
//! texture upload *after* this tap, so [`RobsApp::submit_blackbox_frame`] must
//! be handed the raw BGRA buffer before that swap.
...
impl RobsApp {
    ...
    /// `update()`, before frame processing, so the engine is guaranteed live
    ...
    pub(crate) fn stop_blackbox(&mut self) {
    ...
    pub(crate) fn blackbox_status_chip(&self, ui: &mut eframe::egui::Ui) {
        ...
        let mut tip = format!(
            ...
            Self::format_time(st.total_duration_ms / 1000),
            storage,
        );
